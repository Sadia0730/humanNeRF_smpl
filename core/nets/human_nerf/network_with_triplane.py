...
class Network_Triplane(nn.Module):
    def __init__(
        ...
        self.cnl_mlp = \
            load_canonical_mlp(cfg.canonical_mlp.module)(
                input_ch=cnl_pos_embed_size + tri_feats_size,
    ...
    def _query_mlp(
        ...
        # Copy and normalize pos_flat only for triplane input
        pos_flat_triplane = (pos_flat - pos_flat.min(dim=0, keepdim=True)[0]) / \
                            (pos_flat.max(dim=0, keepdim=True)[0] - pos_flat.min(dim=0, keepdim=True)[0])

        # Extract triplane features using the normalized coordinates
        tri_feats = self.triplane(pos_flat_triplane)
    ...
    def _apply_mlp_kernals(
        ...
        for i in range(0, pos_flat.shape[0], chunk):
            ...
            # Extract the relevant triplane features for the current chunk
            tri_feats_chunk = tri_feats[start:end]
            ...
            # Ensure tri_feats_chunk is on the same device as xyz_embedded
            tri_feats_chunk = tri_feats_chunk.to(xyz_embedded.device)
            print(f"xyz_embedded shape: {xyz_embedded.shape}")
            print(f"tri_feats_chunk shape: {tri_feats_chunk.shape}")
            # Concatenate triplane features with positional embeddings
            combined_features = torch.cat([xyz_embedded, tri_feats_chunk], dim=-1)

            raws += [self.cnl_mlp(
                        pos_embed=combined_features)]
